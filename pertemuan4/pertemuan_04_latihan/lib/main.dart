...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // Jika kosong tampilkan Empty State, jika ada isi tampilkan ListView
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          // Tunggu data yang dikembalikan dari TambahCatatanPage
          final hasil = await Navigator.pushNamed(context, '/tambah');

          // Jika ada data yang dikembalikan, tambahkan ke list _catatan
          if (hasil != null && hasil is Catatan) {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Catatan baru ditambahkan!')),
              );
            }
          }
        },
...
class _TambahCatatanPageState extends State<TambahCatatanPage> {
  ...
  void _simpan() {
    // Validasi form (sudah menangani pesan error merah & minimal 3 karakter)
    if (!_formKey.currentState!.validate()) return;

    // Buat objek Catatan baru dari inputan
    ...
    // Kembali ke Home sambil membawa data catatanBaru
